...
export function createOpenAIProvider(options?: {
  ...
  async function chat(system: string, userContent: unknown, requestOptions?: ProviderRequestOptions): Promise<VisualFinding[]> {
    if (!apiKey) {
      throw new Error('OPENAI_API_KEY not set — pass it via env or createOpenAIProvider({ apiKey })');
    }
    const body = {
      model,
      messages: [
  ...
  return {
    ...
    async review(bundle: ReviewBundle, requestOptions?: ProviderRequestOptions): Promise<VisualFinding[]> {
      ...
      const b64 = Buffer.from(bundle.png).toString('base64');
    ...
    async repair(bundle: ReviewBundle, requestOptions?: ProviderRequestOptions): Promise<VisualFinding[]> {
      ...
      return chat(REPAIR_SYSTEM_PROMPT, user, requestOptions);
    },
